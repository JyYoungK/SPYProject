...
const PictureDetails = ({ toggleModal, openPin }) => {
  return (
    <div className="h-96 w-full flex flex-row bg-white ">
      <div className="mb-8 w-full items-center ">
        <a href={openPin?.links.html} target="_blank">
          <img
            ...
            className="mx-auto my-3 w-10/12 h-full object-cover border-2 shadow-xl items-center shadow-black"
          />
        </a>
      </div>
      <div className=" w-8/12 flex flex-col">
          ...
          <div className="flex flex-row w-full bg-orange-200 pb-3">
            ...
            <div className="flex flex-col ml-5 mt-10">
              <a href={openPin?.user.links.html} target="_blank">
                <div className="font-bold sm:text-lg text-md text-black">
                  {openPin?.user?.name ? openPin?.user.name : "Unknown"}
                </div>
              </a>

              <div className="font-bold sm:text-lg text-md text-gray-500">
                {openPin?.user?.username ? openPin?.user.username : "Unknown"}
              </div>
              {openPin?.user?.for_hire ? (
                openPin?.user?.for_hire ? (
                  <div className="font-bold sm:text-lg text-md text-blue-400">
                    Hire me!
                  </div>
                ) : (
                  <div className="font-bold sm:text-lg text-md text-green-400">
                    Hobbiest
                  </div>
                )
              ) : (
        ...
        <div className="bg-orange-100 h-full flex flex-col ">
          <div className="mt-4 flex flex-row w-full text-gray-600 sm:text-base md:text-lg">
          ...
          <div className="h-full mt-3 mb-2 ml-10 flex flex-col justify-evenly text-black md:text-lg ">
